...
{
    /// <summary>
    /// 用于生成sqlserver数据库的查询语句
    /// </summary>
    public class DbSqlBuild
    {
        #region 函数 BuildPageStr 组建页面底部的页码
        ...
        /// <returns></returns>
        public static string GetPading(string tabName, string queryStrings, string primaryName, string where, int pageSize, int pageIndex, string orderString, string tableSpecialColumn = "", PaddingType padType = PaddingType.NotIn)
        {
            StringBuilder sql = new StringBuilder();

            //if (padType != null && padType.Trim() != "") { padingType = padType.Trim().ToLower();}
            if (!string.IsNullOrWhiteSpace(orderString)) { orderString = " " + orderString; }
            ...
            switch (padType)
            {
                ...
                case PaddingType.RowNumber3:
                    ...
                    sql.AppendFormat("select * from (select row_number() over ({0}) __rowNumber,{1}{2} from {3}) ___a {4}",orderString,queryStrings,tableSpecialColumn,tabName,where);
                    ...
                    sql.AppendFormat(" ___a.__rowNumber between {0} and {1}", skip + 1, skip + pageSize);
                    /*
                    sql.AppendFormat("select {0} {1}{2} from {3} ", pageSize > 0 ? "top " + pageSize : "", queryStrings, tableSpecialColumn, tabName);
                    sql.AppendFormat("{0} ", where);
                    if (skip != 0 && pageIndex > 0 && pageSize > 0)
